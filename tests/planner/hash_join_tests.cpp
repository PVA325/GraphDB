...
//
//   auto join_pred = MakeEq(MakeProp("a", "city_id"), MakeProp("b", "city_id"));
//   auto join = std::make_unique<logical::LogicalJoin>(std::move(left), std::move(right), std::move(join_pred));
//
//   auto residual = MakeGt(MakeProp("a", "age"), MakeProp("b", "age"));
//   auto plan = logical::LogicalPlan(
//       std::make_unique<logical::LogicalFilter>(std::move(join), std::move(residual))
//   );
//
//   optimizer::optimize_logical_plan_impl(plan.root);
//
...
//   EXPECT_EQ(count, 12u);
//
//   // HashJoin должен быть где-то в дереве плана.
//   // Если у тебя residual filter остается сверху, root может быть FilterOp.
//   auto* hash = dynamic_cast<exec::HashJoinOp*>(root.get());
//   if (hash == nullptr) {
